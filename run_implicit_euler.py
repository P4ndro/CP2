"""
ANC Headphones ODE Model - Implicit Euler Method
...
from integrators import solve_implicit_euler
...
def main():
    ...
    print("=" * 60)
    print("Implicit Euler Method - ANC Headphones Model")
    print("=" * 60)
    ...
    for h in config.STEP_SIZES:
        result_fpi = solve_implicit_euler(y0, t_span, h, params, noise_combined,
                                          solver='FPI', tol=config.TOL_FPI, 
                                          max_iter=config.MAX_ITER_FPI)
        result_ngs = solve_implicit_euler(y0, t_span, h, params, noise_combined,
    ...
    h_demo = 0.01
    result_fpi = solve_implicit_euler(y0, t_span, h_demo, params, noise_combined, solver='FPI')
    result_ngs = solve_implicit_euler(y0, t_span, h_demo, params, noise_combined, solver='NGS')
    ...
    axes[0].legend()
    axes[0].set_title(f'Implicit Euler: FPI vs NGS (h = {h_demo})')
    ...
    fig.tight_layout()
    fig.savefig(f"{config.OUTPUT_FIGURES_DIR}/euler_fpi_vs_ngs.png", dpi=150, bbox_inches='tight')
    plt.close(fig)
    
    print("\nDone. Plot saved to results/figures/euler_fpi_vs_ngs.png")
